...
Place recipe files inside `~/.config/fp-appimage-updater/apps/`. Each file can be a flat `.yml` at the top level, or a subdirectory containing a main recipe file and any supporting scripts.
...
<ParamField path="zsync" type="boolean | string">
  ...
  Requires `zsync` to be installed on your `PATH`. Run `fp-appimage-updater doctor` to check availability.
</ParamField>
