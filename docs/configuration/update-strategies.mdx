...
<Tabs>
  <Tab title="Forge">
    Use the `forge` strategy for apps distributed through GitHub or GitLab releases. fp-appimage-updater queries the releases API, matches the asset filename against your pattern, and downloads the matching file.
    ...
    <ParamField path="strategy.repository" type="string" required>
      Full URL to the GitHub or GitLab repository (e.g., `https://github.com/owner/repo`).
    </ParamField>

    <ParamField path="strategy.asset_match" type="string" required>
      Wildcard pattern to match the release asset filename. Uses shell-style glob syntax (e.g., `"*-amd64.AppImage"`). The first matching asset is selected.
    </ParamField>
  ...
  <Tab title="Script">
    ...
    <Note>
      `script_path` is resolved relative to the directory containing the recipe file. If your recipe is at `~/.config/fp-appimage-updater/apps/hayase/main.yml`, then `./resolver.sh` resolves to `~/.config/fp-appimage-updater/apps/hayase/resolver.sh`.
    </Note>
