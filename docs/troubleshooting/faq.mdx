...
<AccordionGroup>
  ...
  <Accordion title="What is the zsync field in an app recipe?">
    The `zsync` field enables delta updates, which download only the bytes that changed instead of the full AppImage. Set it to `true` to let fp-appimage-updater auto-detect the `.zsync` URL, or provide an explicit URL:
    ...
    `zsync` must be installed and available on your `PATH` for this to work. Run `fp-appimage-updater doctor` to check.
  </Accordion>
