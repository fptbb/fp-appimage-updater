...
<CardGroup cols={2}>
  ...
  <Card title="Three update strategies" icon="arrows-rotate">
    Fetch updates from GitHub or GitLab releases with `forge`, a static URL with `direct`, or a custom shell script with `script`.
  </Card>
  <Card title="Delta downloads via zsync" icon="bolt">
    When `zsync` is available on your PATH, the updater downloads only the bytes that changed — saving bandwidth on large AppImages.
  </Card>
  ...
  <Card title="GitHub proxy fallback" icon="shield">
    Retry GitHub release API lookups through one or more configured proxy bases when rate limits are hit, without proxying the actual download.
  </Card>
  <Card title="Single binary under 1 MB" icon="microchip">
    The entire tool ships as one compressed standalone binary with no runtime dependencies.
  </Card>
</CardGroup>
...
<Note>
  The project will never include a recipe repository. You are expected to write and own your own app recipes.
</Note>
