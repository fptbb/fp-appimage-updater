...
<CardGroup cols={2}>
  ...
  <Card title="Three update strategies" icon="arrows-rotate">
    Fetch updates from GitHub, GitLab, Gitea, or Forgejo releases with `forge`, a static URL with `direct`, or a custom shell script with `script`.
  </Card>
  ...
  <Card title="GitHub API tokens & proxies" icon="shield-check">
    Bypass GitHub rate limits using personal access tokens via `secrets.yml` or environment variables. Optional proxy fallback support is also available.
  </Card>
